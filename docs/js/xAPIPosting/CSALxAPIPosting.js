...
var accumlateScore = {
  ...
  TA: { success: 0, failure: 0 },
  Final: { success: 0, failure: 0 },
};
...
var TheSchool = qs("school", "https://csal.autotutor.org");
...
function GetEmail() {
  if (GoogleLogin) {
    ...
    var TheEmail = decodeURIComponent(
      qs("user", qs("UID", "guest@autotutor.org"))
    );
...
function GetFullName() {
  if (GoogleLogin) {
    ...
    var TheFullName = decodeURIComponent(
      qs("fullname", qs("SName", "John Doe"))
    );
...
var LearnerID = { mbox: "mailto:" + user, name: fullname, objectType: "Agent" };

function UpdateTotalScore() {
  totalScore.Hard.failure =
    totalScore.Hard.failure + accumlateScore.Hard.failure;
  totalScore.Medium.failure =
    totalScore.Medium.failure + accumlateScore.Medium.failure;
  totalScore.Easy.failure =
    totalScore.Easy.failure + accumlateScore.Easy.failure;
  totalScore.TA.failure = totalScore.TA.failure + accumlateScore.TA.failure;

  totalScore.Hard.success =
    totalScore.Hard.success + accumlateScore.Hard.success;
  totalScore.Medium.success =
    totalScore.Medium.success + accumlateScore.Medium.success;
  totalScore.Easy.success =
    totalScore.Easy.success + accumlateScore.Easy.success;
...
function CompileScroe(PresentationHistory) {
  ...
  var StudentAnswerType=["Correct","Incorrect"]
  if (StudentAnswerType.includes(PresentationHistory.userAnswer)){
	return PresentationHistory.userAnswer
  }else{
	var AnserHistoy=PresentationHistory.newUserPerfomaceLog;
	var LastHistory=AnserHistoy[AnserHistoy.length-1];
	return LastHistory.userAnswer;
  }
  ...
  // Changes made by XHU 05/26/2021
  if (
    PresentationHistory.newUserPerfomaceLog != null &&
    PresentationHistory.newUserPerfomaceLog.length > 0
  ) {
  ...
  if (inLabel == false) {
    alert(
      "Please take a picture of the screen and send to Dr. Hu. Error message " +
        scoreLabel
    );
    scoreLabel = "";
  }
  ...
  if (scoreLabel == "") {
    ...
    if (userAnswer == "Correct") {
      accumlateScore[scoreLabel].success =
        accumlateScore[scoreLabel].success + 1;
      totalScore[scoreLabel].success = totalScore[scoreLabel].success + 1;
    } else {
      accumlateScore[scoreLabel].failure =
        accumlateScore[scoreLabel].failure + 1;
...
function ComposewithContextActivities(
  AnActor,
  verbObj,
  ResultObj,
  activityObj,
  ...
  if (AContextActivities != []) {
    ...
    contextActivities = {
      grouping: [{ id: SKOSchool }, { id: sessionID }],
      other: [otherObj],
    };
  } else {
  ...
  if (Extdata.extensions[exturl].data.input.PresentationHistory != null) {
    PresentationHistory = JSON.parse(
      Extdata.extensions[exturl].data.input.PresentationHistory
    );
  }
...
function calculateAllScores(response) {
  ...
  for (i = 0; i < response.length; i++) {
    if (response[i].lastScore != null) {
      ThetotalScore.Hard.success =
        ThetotalScore.Hard.success + response[i].lastScore.Hard.success;
      ThetotalScore.Hard.failure =
        ThetotalScore.Hard.failure + response[i].lastScore.Hard.failure;

      ThetotalScore.Medium.success =
        ThetotalScore.Medium.success + response[i].lastScore.Medium.success;
      ThetotalScore.Medium.failure =
        ThetotalScore.Medium.failure + response[i].lastScore.Medium.failure;

      ThetotalScore.Easy.success =
        ThetotalScore.Easy.success + response[i].lastScore.Easy.success;
      ThetotalScore.Easy.failure =
        ThetotalScore.Easy.failure + response[i].lastScore.Easy.failure;
    }
  }
  ...
  AllMedium =
    ThetotalScore.Medium.success.toString() + " of " + Sumall.toString();
...
function GetALLActions(lrsURL, LRSusername, LRSpassword, atimestamp) {
  var queryBody = GetInteractionHistorxAPIJSON(
    LearnerID.mbox,
    TheLessonIDforXAPI.mbox,
    atimestamp
  );
  var settings = {
    url: lrsURL + "statements/aggregate",
  ...
  $.ajax(settings).done(function (response) {
    if (response.length > 0) {
      ...
      var ending = DateString(
        InteractionHistory[InteractionHistory.length - 1].time
      );
      LastActionTime = ending;
      if (starting == ending) {
        ending = " the same day";
      }
      var timemin = GetDuration(
        InteractionHistory[InteractionHistory.length - 1].time,
        atimestamp
      );
      html =
        html +
        "  My record shows that you started this lesson eariler. It was " +
        starting +
        ". You stopped on " +
        ending +
        " " +
...
function getACEActionAndPlay(ActionName) {
  var jsonstr = [
    ...
    { $limit: 100 },
    {
      $project: {
        ContexExt:
          "$statement.context.extensions.https://app.skoonline.org/ITSProfile/CSAL/Data",
      },
    },
    { $project: { Transactions: "$ContexExt.data.response.ACEActions" } },
  ];
  var settings = {
    async: true,
    crossDomain: true,
...
function GetReport(lrsURL, LRSusername, LRSpassword) {
  var queryObj = {
    $and: [
      ...
      { "verb.id": xAPIVerbBase + "action" },
      { $or: [{ "result.success": true }, { "result.success": false }] },
    ],
  };
  var datasqlstring = "actor=" + JSON.stringify(queryObj) + "&limit=1&sort=1";
  var queryStrForSearch =
    "actor=" +
    JSON.stringify({ mbox: LearnerID.mbox }) +
    "&verb=" +
    xAPIVerbBase +
    "action&limit=1";
  $.ajax({
    ...
    success: function (data) {
      if (data.statements.length > 0) {
        ...
        htmlstr =
          htmlstr +
          "<tr><td><b>Question Difficulty Level</b></td><td>Hard<br/>(correct)</td><td>Medium<br/>(correct)</td><td>Easy<br/>(correct)</td></tr>";
        htmlstr = htmlstr + "<td colspan='4'><hr/></td></tr>";

        var TheResult =
          statementGot.result.extensions[
            "https://app.skoonline.org/ITSProfile/CSAL/Result"
          ];
        ...
        var hardTotal =
          The_CurrentScore.Hard.success + The_CurrentScore.Hard.failure;
        htmlstr =
          htmlstr +
          "<td>" +
          The_CurrentScore.Hard.success.toString() +
          " of " +
          hardTotal.toString() +
          "</td>";

        var medianTotal =
          The_CurrentScore.Medium.success + The_CurrentScore.Medium.failure;
        htmlstr =
          htmlstr +
          "<td>" +
          The_CurrentScore.Medium.success.toString() +
          " of " +
          medianTotal.toString() +
          "</td>";

        var easyTotal =
          The_CurrentScore.Easy.success + The_CurrentScore.Easy.failure;
        htmlstr =
          htmlstr +
          "<td>" +
          The_CurrentScore.Easy.success.toString() +
          " of " +
          easyTotal.toString() +
          "</td>";
        ...
        htmlstr =
          htmlstr +
          "<tr><td><b>Scores from all others students <br/> for all lessons</b></td><td><div id='AllHard'></td><td><div id='AllMedium'></td><td><div id='AllEasy'></td></tr>";
...
function Compose(AnActor, verbObj, ResultObj, activityObj, Extdata) {
...
function xAPIPostOther(acePostjson, averb) {
  ...
  if (Assessments.length != 0) {
    statements = ComposewithContextActivities(
      AnActor,
      verbObj,
      ResultObj,
      activityObj,
    ...
    return;
    statements = Compose(AnActor, verbObj, ResultObj, activityObj, Extdata);
  }
  if (statements != null) {
