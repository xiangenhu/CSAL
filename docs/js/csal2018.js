...
function URLToBuncer(tag) {
  ...
  return (
    qs("ATSBase", "https://asatnodejs.uc.r.appspot.com/generate?json=") +
    JSON.stringify(scriptQueryObj)
  );
}
...
function getLessonOutline() {
  ...
  for (i = 0; i < ListOfLessons.length; i++) {
    ...
    if (TheLessonID == theLesson.lessonId) {
      ...
      html =
        html +
        "Click here to test current module: <btton onclick='OpenBrowserTest()' class='btn'>Test Current Module</btton>";
      return html;
    }
  }
}
...
function addRepeatSpeech(agent_speech) {
  ...
  if (currentAgent_Speech.length < 2 || currentAgent_Speech[1].trim() == "")
    return;
...
function GetTheEventAssigned() {
  ...
  $("#btReplayVideo").click(function () {
    if (replayVideo == false) {
      ...
      replayVideo = false;
      vidplayerBusy = false;
      if (SpeakRepeatList.length > 0) ShowRepeatButton();
    }
...
function GetStarted() {
  ...
  LearnerID = { mbox: "mailto:" + user, name: fullname, objectType: "Agent" };
  DataLRS();
  loadAgent();
  showCC(false);
  Learner = sessionStorage.getItem("UID");
...
function restart() {
  ...
  var acePostjson = constructFirstJsontoPut(
    AutoTutorScript,
    currentScripturl,
    currentLessonID
  );
...
function startLesson() {
  ...
  document.getElementById("containerNoImg").style.backgroundImage =
    "url(images/TabletFrame2.png)";
  document.getElementById("containerNoImg").style.backgroundPosition =
    "center bottom";
  document.getElementById("containerNoImg").style.backgroundRepeat =
    "no-repeat";
...
function constructFirstJsontoPut(
  TheAutoTutorScript,
  ThecurrentScripturl,
  ThelessonID
) {
...
function LoadLesson(lessonID) {
  ...
  var aurl =
    "https://class.x-in-y.com/retrieve?json=" + JSON.stringify(retriveObj);
...
function runActions() {
  if (InteractionHistory.length != 0) {
    var lastadata = DateString(
      InteractionHistory[InteractionHistory.length - 1].time
    );
    var thehtml = "<table heigh='100%' width='100%'>";
    thehtml =
      thehtml + "<tr><td><p/> <p/> <h1><p/>Please Wait ....</h1></td></tr>";
    thehtml =
      thehtml +
      "<tr><td><h1>Fast Forwarding to where you have stopped</h1></td></tr>";
    thehtml =
      thehtml +
      "<tr><td valign='middle'><h2>(" +
      lastadata +
      ")</h2></td></tr>";
    thehtml =
      thehtml +
      "<tr><td> steps remaining: " +
      InteractionHistory.length.toString() +
      "</td></tr>";
    thehtml =
      thehtml +
      "<tr><td><button id='stopFF' onclick='stopff()'>Stop</button> </td></tr>";
  ...
  if (actions.length != 0) {
    ...
    currentPageInfor = {
      ACEPostjson: currentJSON,
      PastAFewACEActions: longAction,
      CurrentACEAction: RecordObj,
    };
    ...
    switch (act) {
      ...
      case "Display":
        ...
        if (
          actions[actions.length - 1].Act == "WaitForEvent" &&
          actions[actions.length - 1].Data >= "60"
        ) {
          getQuestionName = data.replace(getUserName, "_user_");
        } else if (
          actions[actions.length - 1].Act == "WaitForEvent" &&
          actions[actions.length - 1].Data == "30" &&
      ...
      case "GetMediaFeedback":
        ...
        document
          ...
          .contentWindow.GetMediaFeedback(data);
...
function Speak2(data) {
  if (talkingheadOn == "true") {
    document
      .getElementById("agentsLarge")
      .contentWindow.callBoth(data, "Speak", "on");
    if (VerboseMode) {
    ...
    document
      .getElementById("agentsLarge")
      .contentWindow.callBoth(data, "Speak", "off");
  }
}
...
function AngentPlay(playList) {
  if (talkingheadOn == true) {
    ...
    document
      .getElementById("agentsLarge")
      .contentWindow.callBoth(playList[0], "Play", "on");
    playList.splice(0, 1);
    ...
    document
      .getElementById("agentsLarge")
      .contentWindow.callBoth(playList[0], "Play", "off");
    playList.splice(0, 1);
...
function nextPage(data) {
  ...
  var currentMediaUrlList2 =
    currentMediaUrlList1[currentMediaUrlList1.length - 1].split("?");
  currentMediaUrl = currentMediaPath + data;
  if (NextButtonOn == true && isHasNextButton == true) {
    if (
      currentMediaUrlList2[0] == "L18-MainPage.html" ||
      currentMediaUrlList2[0] == "L11Round1AMainPage.html"
    ) {
      nextButtonStatus = false;
      HideRepeatButton();
      HidePlayVideoButton();
...
function TrackARCACEAction(TheJSON){
  ...
  var Extdata = {extensions:responseObj};
...
function GetMediaFeedBackMsg(msg) {
  ...
  if (agentFeedBack == "Instruction" || agentFeedBack == "TAGoodAnswer") {
    ...
    agentFeedBack == "SAGoodAnswer" ||
    agentFeedBack == "SABadAnswer" ||
    agentFeedBack == "SABadAnswer"
  ) {
  ...
  for (var i in getMediaObj) {
    ...
    if (getQuestionPath == pageNameList[pageNameList.length - 1]) {
      if (getMediaObj[i].quesionName != undefined) {
        var questionName1 = getMediaObj[i].quesionName
          .trim()
          .toLowerCase()
          .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, "");
        console.log("111111-----------" + getQuestionName);
        var questionName2 = getQuestionName
          .trim()
          .toLowerCase()
          .replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, "");
        if (questionName1.replace(" ", "") == questionName2.replace(" ", "")) {
          if (getType == "QuestionPage") {
            ...
            PresentationHistoryObj.progressBarValue =
              getMediaObj[i].progressBarValue;
            if (
              getMediaObj[i].notCountedItem == false &&
              userAnswer !== "" &&
              waitForUserResponse == true
            ) {
              var d = new Date();
              ...
              CountTotalScore(
                questionName1,
                userAnswer,
                getMediaObj[i].questionLevel,
                PresentationHistoryObj.userSelectedItem,
                PresentationHistoryObj.questionID,
                PresentationHistoryObj.pageStartTimestamp,
                talkingHeadSpeechEndTimestamp,
                userAnswerTimestamp,
                PresentationHistoryObj.userAnswerSpendTime,
                PresentationHistoryObj.progressBarValue
              );
            }
            ...
            PresentationHistoryObj.lessonTextType =
              getMediaObj[i].lessonTextType;
...
function getLastResponse(lrsURL, LRSusername, LRSpassword, Averb) {
  ...
  var secondquery = {
    $and: [{ $sort: { "statement.timestamp": 1 } }, { $limit: 1 }],
  };

  datasqlstring =
    "query=" + JSON.stringify({ $and: [datasqlstringObj, secondquery] });
