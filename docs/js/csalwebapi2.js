...
function LastAction(data) {
  ...
  if (data.ACEActions.length > 0) {
    ...
    for (i = 0; i < data.ACEActions.length; i++) {
      ...
      if (
        theaction.Agent == "System" &&
        (theaction.Act == "Wait" || theaction.Act == "WaitForEvent")
      ) {
        askWait = true;
  ...
  var getUrl = $.ajax({
    ...
    if (InteractionHistory.length == 0) {
      if (data.ACEActions.length > 0) {
        ...
        console.log(
          "No actions returned from ACE. Contact CSAL Team about this error!"
        );
        if (qs("STOPWhenEmptyACEAction", "0") == "1") {
          StopTimer();
          return;
        }
      }
