...
function GetScript(lessonID) {
  ...
  if (talkingheadLoaded == false) {
    var url =
      scriptFolderURL + lessonID + "/html5/index.html?lessonName=" + lessonID;
...
function vidPlayerControl(vidId) {
  ...
  if (VideoType == 2) {
    ...
    document
      .getElementById("vid")
      .contentWindow.myLocalVideoPlayer(Vid, replayVideo);
  }
}
...
function stopPlayVideo() {
  ...
  document
    .getElementById("vid")
    .contentWindow.myLocalVideoPlayer("", replayVideo);
  vidplayerBusy = false;
  $("#videoDiv").css("z-index", "-10");
  $("#videoDiv").css("visibility", "hidden");
}
