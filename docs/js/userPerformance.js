...
function CountTotalScore(
  ...
  if (
    ...
    if (TotalScoreArr.length > 0) {
      ...
      TotalScoreObj.talkingHeadSpeechEndTimestamp =
        talkingHeadSpeechEndTimestamp;
...
function showEndingPage() {
  ...
  if (lessonID == "lesson1") {
    ...
    for (var i in TotalScoreArr) {
      if (
        TotalScoreArr[i].userAnswer == "Correct" &&
        TotalScoreArr[i].AnswerTime == 1
      ) {
        countCrrectNum++;
      }
    ...
    for (var i in TotalScoreArr) {
      if (
        TotalScoreArr[i].pagePath != "Lesson25-Main0-1.html" ||
        TotalScoreArr[i].pagePath != "Lesson25-Main1-2.html" ||
        TotalScoreArr[i].pagePath != "Lesson25-Main2-1.html"
      ) {
        if (
          TotalScoreArr[i].userAnswer == "Correct" &&
          TotalScoreArr[i].AnswerTime == 1
...
function userPerformancePage(result, rawScore, min, max, TextLevel) {
  ...
  if (result == "pass") {
    $("#mainFrame").attr(
      "src",
      "resources/endingpage.html?userPerformance=Passed"
    );
    xAPIPostEnding(
      qs("UID", ""),
      qs("SName", ""),
    ...
    $("#mainFrame").attr(
      "src",
      "resources/endingpage.html?userPerformance=Failed"
    );
    xAPIPostEnding(
      qs("UID", ""),
      qs("SName", ""),
